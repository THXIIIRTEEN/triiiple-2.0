...
const Message = forwardRef<HTMLDivElement, IMessage>(({
  ...
  const { _id: authorId, username, tag } = author;
  ...
  return (
    <div className={`${styles.messageBlock} ${authorId === profile?.id && styles.currentUser}`} ref={mergeRefs([ref, messageRef])}>
      <UserAvatar id={authorId} className={styles.avatar}/>
      <div className={styles.textBlock}>
        <div className={styles.top}>
          { authorId === profile?.id &&
            <p className={styles.username}>Вы</p>
          }
          { authorId !== profile?.id && tag &&
        ...
        { mediaFiles.length > 0 && <div className={styles.photoCollage}>{mediaFiles.length > 0 && <PhotoCollage photos={mediaFiles} />}</div>}
        {!editMode ? (
        ...
        <div className={styles.bottom}>
          {isEdited && <p className={styles.isRead}>Ред.</p>}
          {!isRead && 
            <svg width="18" height="16" viewBox="0 0 18 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd" d="M17.5867 0.22126C18.0502 0.580438 18.1369 1.25014 17.7803 1.71708L7.19213 15.5838C7.00642 15.827 6.72562 15.9779 6.42153 15.9977C6.11749 16.0177 5.81966 15.9046 5.60421 15.6877L0.310118 10.3543C-0.103373 9.9377 -0.103373 9.26239 0.310118 8.84575C0.723618 8.42922 1.39402 8.42922 1.80752 8.84575L6.24892 13.3201L16.1019 0.416364C16.4584 -0.0505799 17.1232 -0.137929 17.5867 0.22126Z" fill="black"/>
            </svg>
          }
          {isRead && 
            <svg width="26" height="16" viewBox="0 0 26 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd" d="M17.5867 0.22126C18.0502 0.580438 18.1369 1.25014 17.7803 1.71708L7.19213 15.5838C7.00642 15.827 6.72562 15.9779 6.42153 15.9977C6.11749 16.0177 5.81966 15.9046 5.60421 15.6877L0.310118 10.3543C-0.103373 9.9377 -0.103373 9.26239 0.310118 8.84575C0.723618 8.42922 1.39402 8.42922 1.80752 8.84575L6.24892 13.3201L16.1019 0.416364C16.4584 -0.0505799 17.1232 -0.137929 17.5867 0.22126Z" fill="black"/>
              <path fill-rule="evenodd" clip-rule="evenodd" d="M25.5897 0.22126C26.0498 0.580438 26.1359 1.25014 25.782 1.71708L15.2721 15.5838C15.0878 15.827 14.8091 15.9779 14.5072 15.9977C14.2054 16.0177 13.9098 15.9046 13.696 15.6877L11.3078 13.3181C10.8974 12.9015 10.8974 12.2262 11.3078 11.8095C11.7183 11.393 12.3837 11.393 12.7941 11.8095L14.3359 13.3201L24.1159 0.416364C24.4698 -0.0505799 25.1297 -0.137929 25.5897 0.22126Z" fill="black"/>
            </svg>
          }
        </div>

        {authorId === profile?.id && showButtons && (
